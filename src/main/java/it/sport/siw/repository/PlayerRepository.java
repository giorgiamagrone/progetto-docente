...
public interface PlayerRepository extends CrudRepository<Player, Long>{
		...
		Optional<Player> findByIdWithContract(@Param("id") Long id);
		  Optional<Player> findByIdAndTeamIsNotNull(Long id);
		@Query("SELECT p FROM Player p WHERE p.name = :name AND p.surname = :surname AND p.dateOfBirth = :dateOfBirth")
		List<Player> findPlayerByDetails(@Param("name") String name, @Param("surname") String surname, @Param("dateOfBirth") LocalDate dateOfBirth);
